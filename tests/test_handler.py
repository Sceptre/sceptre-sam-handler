...
class TestSAM(FsTestCase):
    def setUp(self):
        ...
        self.invoker = Mock(
            **{"spec": SamInvoker, "invoke.side_effect": self.fake_invoke}
        )
        ...
        self.render_jinja_template: Mock = create_autospec(
            helper.render_jinja_template, return_value=self.processed_contents
        )
    ...
    def fake_invoke(self, command, args):
        if command == "build":
            self._is_built = True
            self.assertTrue(Path(args["template-file"]).exists())
    ...
    def test_handle__instantiates_invoker_with_correct_args(self):
        ...
        self.invoker_class.assert_called_with(
            self.connection_manager, Path(self.arguments["path"]).parent.absolute()
        )
    ...
    def test_handle__invokes_package_with_default_arguments(self):
        ...
        expected_prefix = "/".join(
            [self.arguments["artifact_prefix"], *self.name.split("/"), "sam_artifacts"]
        )
    ...
    def test_handle__package_args_specified__invokes_package_with_default_arguments(
        self,
    ):
    ...
    def test_validate_args_schema(self):
        self.arguments["build_args"] = {"use-container": True}
        self.arguments["package_args"] = {"region": "us-east-1"}
        self.handler.validate()

    def test_handle__path_has_jinja_extension__renders_template_with_correct_parameters(
        self,
    ):
    ...
    def test_handle__path_has_jinja_extension__invokes_build_with_compiled_jinja_template_path(
        self,
    ):
        self.arguments["path"] = "my/random/path.yaml.j2"
        ...
        self.handler.handle()
        self.invoker.invoke.assert_any_call(
            "build", {"cached": True, "template-file": str(expected_file_path)}
        )
    ...
    def test_handle__path_has_jinja_extension_and_skip_jinja_cleanup_flag_is_true__keeps_compiled_jinja_file(
        self,
    ):
        self.arguments["path"] = "my/random/path.yaml.j2"
    ...
    def test_handle__path_has_jinja_extension__returns_contents_of_destination_template_file(
        self,
    ):
        result = self.handler.handle()
        self.assertEqual(self.processed_contents, result)

    def test_handle__unsupported_path_extension__raises_unsupported_file_type_error(
        self,
    ):
...
class TestSamInvoker(TestCase):
    def setUp(self):
        ...
        self.credentials = Mock(
            spec=Credentials, access_key="access", secret_key="secret", token="token"
        )
